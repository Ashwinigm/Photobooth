...
def firestore():
    ...
    # Ensure no photos exist before running the tests. This typically helps if
    # tests somehow left the database in a bad state.
    delete_all_photos(firestore)

    yield firestore

    # Delete all photos that we created during tests.
    delete_all_photos(firestore)


def delete_all_photos(firestore):
    while True:
        photos, _ = firestore.next_page(limit=50)
        if not photos:
            break
        for photo in photos:
            firestore.delete(photo['id'])


def test_list(app, firestore):
    for i in range(1, 12):
        firestore.create({'title': u'Photo {0}'.format(i)})
    ...
    assert 'Photo 1' in body, "Should show photos"
    assert len(re.findall('<h4>Photo', body)) <= 10, (
        "Should not show more than 10 photos")
...
def test_add(app):
    data = {
        ...
        'takenDate': 'Test Date Taken',
        'location': 'Test Location'
    }

    with app.test_client() as c:
        rv = c.post('photos/add', data=data, follow_redirects=True)

    assert rv.status == '200 OK'
    body = rv.data.decode('utf-8')
    assert 'Test photo' in body
    assert 'Test Photographer' in body
