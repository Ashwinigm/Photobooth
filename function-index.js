...
exports.vision_analysis = async (event, context) => {
    ...
    if (response.error === null) {
        ...
        if (isSafe) {
            const pictureStore = new Firestore().collection(u'Photo');
            
            // const doc = pictureStore.doc(filename);
            var URL = `https://storage.googleapis.com/${filebucket}/${filename}`
            const doc = pictureStore.where(u'imageUrl', '==', URL).get();
            if (!doc.exists) {
                console.log('No such document!');
                throw new Error(`Not able to find doc using URL ${URL}`);
            } else {
